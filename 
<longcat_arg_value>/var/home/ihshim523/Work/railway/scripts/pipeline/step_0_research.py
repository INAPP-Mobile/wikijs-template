...
from .core import (
    call_worker, deterministic_review, fail, ok, print_go_nogo,
    read_file_content, section_divider, step_header, sub_step, warn,
)


def step_0_research(template_dir: str, workspace: str) -> bool:
    step_header("Step 0", "Static Checks + Duplicate Detection")
    ...
    if _is_duplicate(name, published):
        fail(f"'{name}' duplicates published template: {_find_matches(name, published)}")
        return False
...
def _detect_blockers(content: str) -> list[str]:
    ...
    if ":latest" in content.lower():
        out.append(":latest tag — pin version")
    if "USER " not in content and "USER\t" not in content:
        out.append("No USER directive")
...
def _find_matches(name: str, published: list) -> list[str]:
    matches = []
    normalized = re.sub(r'[\s_\-]+', '-', name.lower().strip())
    for t in published:
        for key in ("name", "slug", "code"):
